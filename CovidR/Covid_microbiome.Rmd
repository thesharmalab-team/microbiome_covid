...
# Covid data set
data_hosp <- read.csv("Covid_Curat_pop.csv")

# COVID Range of the graphs
...
#cowid.owid<- covid.owid[order(covid.owid[,'location']), ]
```
...
###########
  ...
  # stats for age:
  # for whole dataset
  a <- summary(data$age_category)
  b <- sd(data$age_category,na.rm=T)
  # by group
...
alpha.p2.curat <<- alpha.p2.curat + stat_compare_means(comparisons = covid.pairs) + 
    ...
    guides(fill=guide_legend(title="Covid Hospitalisations", title.theme = element_text(size = 16))) + 
...
fig.alpha.curat.group <- alpha.p1.curat + alpha.p2.curat +
  ...
  subtitle = 'Individual Countries and resulting Disease Group') & theme(legend.position = 'bottom', legend.text = element_text(size = 16))
# Saves the output
...
save_plot(paste(output.prefix, "figure.abdunace.png", sep="_"), p.abdunace, ncol = 2, base_height = 11.69, base_width = 9.5) 
...
p <- plot_ordination(beta_curat, ordinated_taxa, color="curat.covid.class", 
                title = "Bray-Curtis Principal Coordinates Analysis") + theme(axis.title.y = element_text(size = 16)) +
   theme(axis.title.x = element_text(size = 16)) + 
   guides(fill=guide_legend(title="Covid Hospitalisations", title.theme = element_text(size = 16)))
...
permanova.curat <- mypermanova[["permanova.curat"]]
  ...
  p1 <- ggplot(permanova.curat,aes(reorder(label, top.coef),top.coef,fill = top.coef > 0)) +
    ...
    annotate("text", x = 2.5, y = 0.01, label = "Harmful Taxa") + 
