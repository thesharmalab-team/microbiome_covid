...
# Covid data set
data_hosp_pop <- read.csv("~/Desktop/Covid_Curat_pop.csv")
data_hosp <- read.csv("~/Desktop/Covid_Curat.csv")
data_hosp[1] <- NULL
#get data hosp_by_pop in main csv 
hosp_by_pop <- data_hosp_pop$hosp_by_pop
data_hosp <- cbind(data_hosp, hosp_by_pop)
...
covid.owid <- as.data.frame(data_hosp, col_names = TRUE) %>%
  rename_all( tolower)  %>%
  dplyr::select(location, hosp_patients, aged_70_older, gdp_per_capita, human_development_index, hosp_by_pop, population)
...
curat.tabl1 <- my_cur_table(curat.recode.country.pseq)
# Prints it
curat.tabl1 


# Generates the table. This is a Sharmalab function
my_mgnify_table <- function(psobj_gut_USA){
  
  # subset this pseq
  mgnify.pseq <- subset_samples(psobj_gut_USA, mgnify.ad.class == "HIGH" | mgnify.ad.class == "LOW")
  
  #get the length of the total numbers
  n <- length(sample_data(psobj_gut_USA)$mgnify.ad.class)
  data <- data.frame(id=1:n)
  
  # Get the classificationo
  data$mgnify.ad.class <- sample_data(psobj_gut_USA)$mgnify.ad.class
  
  #Get the covariates
  #Gender
  data$gender <- sample_data(psobj_gut_USA)$sample_sex
  # replace the NA with unknown
  data$gender <- fct_explicit_na(data$gender, na_level = "unknown")
  
  t1 <- data %>%  set_labels(list(gender = "Gender")) %>% 
    tableby(mgnify.ad.class ~ gender, 
            data = ., 
            test=FALSE,
    ...
    as.data.frame() %>% dplyr::select(.,
                               ...
                               -variable.type,
                               -variable) %>%
    dplyr::rename(., "Variable" = label, "Low AD prevalence Group" = LOW, "High AD prevalence group" = HIGH)
...
curat.tabl1 <-my_curat_table(mypseq)
...
# Saves the output
save_plot(paste(output.prefix, "figure.alpha.group.png", sep="_"), fig.alpha.group, ncol = 2, base_height = 11.69, base_width = 8.2)  
