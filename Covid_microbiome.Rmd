...
c1 <- ggplot(mydata[["curat.combined.agestd" ]], aes(x = year, y = DALY, colour = location)) + 
  geom_line() + 
...
##################### Creates captions #######################
temp <- subset(mydata[["atlas.curat.combined.agestd" ]], year==gbd.year)
...
# Curat dataset
mydata[["curat.combined" ]]$hl.label[mydata[["curat.combined" ]]$Prevalence >= mythres] <- "HIGH"
mydata[["curat.combined" ]]$hl.label[mydata[["curat.combined" ]]$Prevalence < mythres] <- "LOW"
...
ct4 <- ggplot(subset(mydata[["curat.combined" ]], year==gbd.year & age == gbd.age), aes( area=Freq,fill=hl.label, label = curat.label )) + 
  geom_treemap() + 
  scale_color_scico(palette = "vik")  +
  geom_treemap_text( colour = "black", place = "centre", grow = FALSE) + 
  theme(legend.position="bottom") + 
  labs(fill = mnd.incid.label )

# Combines the Curat figures
figure.curat.class <- c1 + ct1 + c2 + ct2  + c3 + ct3 +  plot_spacer() + ct4 + 
  plot_layout(ncol = 2) + plot_annotation(
  title = fig_title_gbm_curat,
